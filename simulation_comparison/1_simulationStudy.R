...
##############################
# see https://github.com/salleuska/CPLogitx
...
## IP code
## priors for informed partition model
 ...
 Ymat <- NULL
  ...
  cat("informed partition model \n")
    drpm1 <- drpm_fit(y=Ymat, 
                    ...
                    initial_partition = trueP, ## initial partition, ## If NULL we have a DP process prior
...
#                   phi1_0 = TRUE,
    ...
    res <- gibbsDensityCP(X_list, Y_list, Z_prior = trueP -1, psi_par = centeredPartPsis[j],
   ...
   cat("compute ARI, LPML, WAIC \n")
    ...
    ariMatInitialIP <- ariMatInitialCP <- matrix(NA, nout, Tm)
    ariMatTrueIP <- ariMatTrueCP <- matrix(NA, nout, Tm)
    
    for(m in 1:nout){
      for(t in 1:Tm) {
        ariMatInitialIP[m,t] <- adjustedRandIndex(drpm1$Si[t,,m], trueP[,t])
        ariMatTrueIP[m,t] <- adjustedRandIndex(drpm1$Si[t,,m], initialP)

        ariMatInitialCP[m,t] <- adjustedRandIndex(clustSamp[m, ], trueP[,t])
        ariMatTrueCP[m,t] <- adjustedRandIndex(clustSamp[m,], initialP)        
      }
    }
